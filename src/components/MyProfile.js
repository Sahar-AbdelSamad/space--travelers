...
const Profile = () => {
  const rockets = useSelector((state) => state.rockets.filter((item) => item.reserved));
          ...
          {rockets.map((rocket) => (
            <tbody key={rocket.id}>
              <tr className="res-rocket">
                ...
                <td className="td">
                  <button type="button" className="button button-reserve" onClick={() => dispatch(toggleReservation(rocket.id))}>Cancel Reservation</button>
                </td>
                <td className="td">
